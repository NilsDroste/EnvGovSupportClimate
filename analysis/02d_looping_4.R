...
timeseriesplot4 <- preds4  %>% 
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 9),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 10),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 11),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 12),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 13),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 14),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 15),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 16),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
  ...
  geom_line(
    data = preds4 %>% filter(PolicyIndicator == 17),
    aes(y = fit, x = EndCollection),
    col = "darkred"
  ) +
  geom_ribbon(
...
j = data4 %>% pull(PolicyIndicator) %>% as.numeric() %>% min(na.rm = T)
...
for (i in (envpols[c(9, 11:13, 15:17), ] %>% rownames() %>% as.numeric() %>% min):(envpols[c(9, 11:13, 15:17), ] %>% rownames() %>% as.numeric() %>% max)) {
  ...
  plot_lst4[[i]] <- df %>% filter(!is.na(PolicyIndicator))  %>%
    ggplot(aes(y = GovernmentSupport, x = EndCollection)) +
...
plot4 <- cowplot::plot_grid(
  timeseriesplot4,
  cowplot::plot_grid(
    cowplot::plot_grid(plotlist = plot_lst4, nrow = 1),
    cowplot::plot_grid(plotlist = plot_lst4_resids, nrow = 1),
    nrow = 2,
    align = "v",
    axis = "lr"
  ),
  cowplot::plot_grid(plotlist = coef_plot_lst4, nrow =
                       1),
  labels = c('a)', 'b)', 'c)'),
  nrow = 3
)
