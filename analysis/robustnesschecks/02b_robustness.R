...
for (j in c(60,90,120)) {
  ...
  for (i in (envpols[4:5, ] %>% rownames() %>% as.numeric() %>% min):(envpols[4:5, ] %>% rownames() %>% as.numeric() %>% max)) {
    ...
    df <-
      data2 %>% filter(
        Published >= lubridate::ymd(envpols[4:5, ]$date[i]) - j &
    ...
    df_resids <-
      preds2 %>% filter(
        StartCollection >= lubridate::ymd(envpols[4:5, ]$date[i]) - j &
  ...
  bandwidth_lst2[[bw]] <- coef_lst2
  bandwidth_plot_lst2[[bw]] <- coef_plot_lst2
  
}


# Plotting
plot2_bw <- cowplot::plot_grid(
  cowplot::plot_grid(
    plotlist = bandwidth_plot_lst2[[1]], ncol = 2, align = "h"),
  cowplot::plot_grid(
    plotlist = bandwidth_plot_lst2[[2]], ncol = 2, align = "h"),
  cowplot::plot_grid(
    plotlist = bandwidth_plot_lst2[[3]], ncol = 2, align = "h"),
